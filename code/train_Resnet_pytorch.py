...
model = ResNet(BasicBlock, [3,3,3]).double().to(device)
if device=='cuda':
...
print("Loading...")
#checkpoint = torch.load('../models/best_model_Resnet.h5')
...
if __name__ == "__main__":
    ...
    best_model_path = '../models/best_model_Resnet_0.01_nonsilence.h5'
    loss_l = []
    ...
    plt.savefig('../results/Resnet_mfcc_Adam_0.01_nonslience.png')
    plt.figure(2)
    plt.plot(np.arange(0, EPISODE-1, 1), test_acc)
    plt.show()
    plt.savefig('../results/Resnet_mfcc_Adam_0.01_nonsilence_test_acc.png')
