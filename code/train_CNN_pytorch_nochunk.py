...
class CNN(nn.Module):
    def __init__(self, num_classes=4):
        ...
        self.conv1 = nn.Conv2d(1, 8, kernel_size=13, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(8)
        self.conv2 = nn.Conv2d(8, 8, kernel_size=13, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(8)
        self.conv3 = nn.Conv2d(8, 8, kernel_size=13, stride=1, padding=1, bias=False)
        self.bn3 = nn.BatchNorm2d(8)
        self.conv4 = nn.Conv2d(8, 16, kernel_size=2, stride=1, padding=1, bias=False)
        self.bn4 = nn.BatchNorm2d(16)
        self.flatten = Flatten()
        self.linear1 = nn.Linear(6400,16)
...
checkpoint = torch.load('../models/best_model_CNN_mfcc_50mc_nochunk_nonsilnece.h5')
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        data = data.view((len(data),1 , 198,39))  #(B, C, H, W): (32, 1, 198, 39)
        #print(data.shape) 
...
if __name__ == "__main__":
    ...
    best_model_path = '../models/best_model_CNN_mfcc_50mc_nochunk_nonsilnece.h5'
    ...
    plt.savefig('../results/CNN_mfcc_nonsilence_nochunk_50ms.png')
    plt.figure(2)
    plt.plot(np.arange(0, EPISODE-1, 1), test_acc)
    plt.show()
    plt.savefig('../results/CNN_mfcc_nonsilence_nochunk_50ms_test_acc.png')
