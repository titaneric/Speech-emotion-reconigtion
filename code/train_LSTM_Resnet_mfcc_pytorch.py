...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        data = data.view((len(data),-1,39)).double() #(batch_size, seq_len, input_size) #(B, C, H, W): (32, 1, 198, 39)
...
def test():
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(test_loader):
            data = data.view((len(data),-1,39)).double()
            data, target = data.to(device), target.to(device)
            output = model(data)
            #print("output of test: ", output, output.size())
            loss = F.cross_entropy(output, (torch.max(target, 1)[1]))
...
if __name__ == "__main__":
    ...
    best_model_path = '../models/best_model_LSTM_Resnet_mfcc.h5'
    loss_l = []
    ...
    plt.savefig('../results/LSTM_Resnet_mfcc.png')
    plt.figure(2)
    plt.plot(np.arange(0, EPSIODE-1, 1), test_acc)
    plt.show()
    plt.savefig('../results/LSTM_Resnet_mfcc_test_acc.png')
