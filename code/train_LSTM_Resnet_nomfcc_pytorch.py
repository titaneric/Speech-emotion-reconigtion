##############################################
# LSTM + Resnet
# with compressed, slience-removal and normalized dataset
# 
##############################################
import numpy as np
import matplotlib
...
from utilities_LSTM import get_data, class_labels
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.layer1(out)
        #out = F.avg_pool2d(out, kernel_size=(2,1))
        out = self.layer2(out)
        #out = F.avg_pool2d(out, kernel_size=(2,1))
        out = self.layer3(out)
        #print("resnet layer3: ",out.shape)
        out = F.avg_pool2d(out, kernel_size = (8,8))
        #print("resnet: ",out.shape)
        out = out.view(out.size(0), -1)
        #print("resnet linear: ",out.shape)
        out = self.linear(out)
        return out
...
class RNN(nn.Module):
    def __init__(self, block, num_blocks, num_classes=4):
        ...
        #self.resnet = ResNet(BasicBlock, [3,3,3]).double().to(device)
    ...
    def forward(self, x):
        ...
        r_out = r_out.view((len(r_out), 1, -1, 64))
        #print(r_out)
        #r_out = r_out[:, -1, :]
        #print(r_out.shape)
        out = F.relu(self.bn1(self.conv1(r_out)))
        out = self.layer1(out)
        #out = F.avg_pool2d(out, kernel_size=(2,1))
        out = self.layer2(out)
        #out = F.avg_pool2d(out, kernel_size=(2,1))
...
model = RNN(BasicBlock, [3,3,3]).double().to(device)
if device=='cuda':
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        data = data.view((len(data),-1,3001)).double() #(batch_size, seq_len, input_size) #(B, C, H, W): (32, 1, 198, 39)
...
def test():
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(test_loader):
            data = data.view((len(data),-1,3001)).double()
            data, target = data.to(device), target.to(device)
            output = model(data)
            #print("output of test: ", output, output.size())
            loss = F.cross_entropy(output, (torch.max(target, 1)[1]))
            ...
            #print("pred of test: ", pred, pred.size())
            total += target.size(0)
...
if __name__ == "__main__":
    ...
    # print("x_train: ", x_train.shape) # (246,180060)->(dataset size, input_size)
    ...
    plt.savefig('../results/LSTM_Resnet.png')
    plt.figure(2)
    plt.plot(np.arange(0, EPSIODE-1, 1), test_acc)
    plt.show()
    plt.savefig('../results/LSTM_Resnettest_acc.png')
